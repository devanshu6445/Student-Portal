...
interface AnnouncementDao {
    ...
    @Query("SELECT * FROM announcements WHERE (announcementTimestamp+86000) > :currentTimestamp ORDER BY CASE announcementType WHEN 'Universal' THEN 0 WHEN 'Course' THEN 1 WHEN 'Class' THEN 2 END, announcementTimestamp DESC LIMIT 1" )
    fun getLatestAnnouncement(currentTimestamp:Long) : Flow<AnnouncementEntity>

    @Query("SELECT * FROM announcements ORDER BY CASE announcementType WHEN 'Universal' THEN 0 WHEN 'Course' THEN 1 WHEN 'Class' THEN 2 END, announcementTimestamp DESC" )
